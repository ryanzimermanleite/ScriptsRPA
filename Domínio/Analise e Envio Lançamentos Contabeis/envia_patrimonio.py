...
def login(empresa, andamentos):
    ...
    while _find_img('trocar_empresa.png', pasta='imgs_c', conf=0.9):
        ...
        if _find_img('conforme_modulo.png', pasta='imgs_c', conf=0.9):
            _escreve_relatorio_csv(';'.join([cod, cnpj, 'empresa erro modulo']),
                                   nome=andamentos)
            print('❌ Empresa ERRO MODULO')
            p.press('enter')
            time.sleep(1)
            p.press('esc', presses=5)
            while _find_img('trocar_empresa.png', pasta='imgs_c', conf=0.9):
...
def patrimonio(empresa, andamentos):
    cod, cnpj, status = empresa

    if status == 'Parametro Errado':
        _escreve_relatorio_csv(';'.join([cod, cnpj, 'Parametro Errado']), nome=andamentos)
        return 'ok'
    else:
        ...
        if _find_img('aviso_cadastro.png', conf=0.9):
            ...
            time.sleep(0.5)
            p.press('y')
            while _find_img('patrimonio_azul.png', conf=0.9):
                time.sleep(1)
            time.sleep(2)
            if _find_img('ass_cliente.png', conf=0.9):
                associacao_cliente = 'ok'
                p.press('enter')
                p.press('esc', presses=5)
                _escreve_relatorio_csv(';'.join([cod, cnpj, 'Erro Imune - Associação']), nome=andamentos)
                return 'ok'
            p.press('r')
            time.sleep(2)
            p.press('enter')
        ...
        if _find_img('aviso_fornecedor.png', conf=0.9):
            p.press('enter')
            fornecedor = 'ok'
        else:
            p.press('t')
            ...
            time.sleep(0.5)
            p.press('y')
            while _find_img('patrimonio_forne_azul.png', conf=0.9):
                time.sleep(1)
            time.sleep(2)
            if _find_img('ass_fornecedor.png', conf=0.9):
                associacao_fornecedor = 'ok'
                p.press('enter')
                p.press('esc', presses=5)
                _escreve_relatorio_csv(';'.join([cod, cnpj, 'Erro Imune - Associação']), nome=andamentos)
                return 'ok'
        ...
        if cliente == 'ok' and fornecedor == 'ok':
            p.press('esc', presses=5)
            _escreve_relatorio_csv(';'.join([cod, cnpj, 'Patrimonio OK']), nome=andamentos)
            return 'ok'
        else:
            p.press('esc', presses=5)
            _escreve_relatorio_csv(';'.join([cod, cnpj, 'Patrimonio Errado']), nome=andamentos)
            return 'ok'
...
def run(window):
    ...
    andamentos = 'Resultado_Patrimonio'
    ...
    for count, empresa in enumerate(empresas[index:], start=1):
        ...
        while True:
            ...
            resultado = patrimonio(empresa, andamentos)
