...
diretorioArquivosTxt = 'V:\!Setor SPED\EFD Fiscal\EFD Arquivos'
...
def localizaPastaArquivosTxt():
    while not _find_img('pasta_EFD_arquivo_selecionada.png', pasta='imgs', conf=0.9):
        ...
        time.sleep(0.5)
        _click_img('EFD_Fiscal.png', pasta='imgs_fiscal', conf=0.9, clicks=2)
        time.sleep(0.5)
...
def verificaMsgImportacao():
    ...
    while not _find_img('resultado_validacao.png', pasta='imgs', conf=0.9):
        ...
        if _find_img('atualizacao_tabela.png', pasta='imgs', conf=0.9):
            p.press('enter')
        if _find_img('arquivo_contem_avisos_fiscal.png', pasta='imgs_fiscal', conf=0.9):
            p.press('enter')
        if _find_img('arquivo_validado_sucesso_fiscal.png', pasta='imgs_fiscal', conf=0.9):
            p.press('enter')
...
def verificaMsgAssinatura():
    while not _find_img('resultado_processamento_fiscal.png', pasta='imgs_fiscal', conf=0.9):
        time.sleep(0.5)
    p.press('esc')
    time.sleep(0.5)
    p.press('esc')
...
def geraRelatorioExcel(arquivo):
    ...
    dataCompetencia = dados_arquivo[4]
    razaoSocial = dados_arquivo[5]
    cnpjCliente = dados_arquivo[6]
    ...
    nomeRelatorioExcel = 'Relatorio Fiscal ' + dataCompetencia[2:4] + '-' + dataCompetencia[4:]
